...
# remove generated file to reduce clutter
unlink("test.png")
```
...
# fit single model in parallel with ctmm.fit. This is not used in later anlaysis
model_fit_res <- par_fit_models(data_sample)
...
### Create reproducible example for bug report

- Sometimes you may met error with certain data, especially in model selection. To report the bug and help track down the problem, it'll be much more helpful to create a reproducible example. You can use package functions to reproduce the steps in the webapp.

  Below is some simple code to import data and try model selection on it.
```{r code example to reproduce problems}
library(ctmm)
library(ctmmweb)

# single animal data will be imported as telemetry object by default, use `drop` parameter to keep result as a list
tele <- as.telemetry("d:/Work/R/ctmm-shiny/data/Exported_2019-09-21_22-22-11.csv", drop = FALSE)
model_try_res <- par_try_models(tele)
model_summary <- summary_tried_models(model_try_res)
```
