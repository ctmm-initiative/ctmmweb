...
# basic data structure
combined_data <- combine(buffalo)
# a list of locations data.table/data.frame and information table
loc_data <- combined_data$data
info <- combined_data$info
```
...
```{r outlier}
# distance_center column is added after calculation. Note the telemetry list is needed for error information
loc_data_distance <- calc_distance(loc_data, buffalo)
...
loc_data_speed <- calc_speed(loc_data_distance, buffalo)
...
# move output to temporary folder
file.rename("point_map.html", "fig/point_map.html")
```
